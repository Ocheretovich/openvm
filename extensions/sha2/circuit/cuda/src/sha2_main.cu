...
// kernel's frame (SHA-256, where the kernel was already tight at 16 B stack)
// or in a separate function frame (SHA-512, where the inlined version was
// driving ~1.2 KiB of stack and ~1.3 KiB of spills).
template <typename V>
...
// STATE_WRITES=16) and the inlined body was producing ~1.2 KiB of stack frame
// and ~1.3 KiB of register spills in the kernel. Outlining gives the body its
// own stack frame and drops the kernel's footprint dramatically.
template <typename V>
...
__global__ void sha2_main_tracegen(
    ...
    // working set out of the kernel's stack frame; SHA-256 inlines the body
    // directly because it was already register-resident at 16 B stack and
    // outlining only added local-memory traffic.
